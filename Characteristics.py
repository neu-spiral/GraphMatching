...
def paths_and_cycles(graph, N, k):
    ...
    while i < k:
        ...
        print "Avg. number of paths and cycles are", all_cycles_and_paths.values()\
                                                    .map(lambda d: tuple([d[key] for key in d]))\
                                                    .map(lambda (paths_list, cycles_list):(np.array(paths_list)/number_of_nodes, np.array(cycles_list)/number_of_nodes))\
                                                    .reduce(lambda (paths_array1, cycles_array1), (paths_array2, cycles_array2):(paths_array1+paths_array2,cycles_array1+cycles_array2))
        if checkpnt:
...
if __name__ == "__main__":
    ...
    parser.add_argument('mode', help = 'Option of what is being calculated (i.e. neighborhood size (NH), cycles and '
                                     'paths (CP), page rank (PR).', choices = ['NH','CP', 'PR'])
    ...
    if args.mode == 'NH':
        neighbors = neighborhood(graph, args.N, args.k)
