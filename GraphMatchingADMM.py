...
if __name__=="__main__":
    ...
    if args.partitionR != None:
        ...
        logger.info("Loaded partitioning function for variables.")
    if args.partitionL != None:
        partDictL = readJSON(args.partitionL)
        SijpartFunc = lambda x: partDictL[x]
        logger.info("Loaded partitioning function for objectives.")
    else:
        SijpartFunc = None
    ...
    if args.initRDD != None:
       ...
       #Resume iterations from prevsiously dumped iterations. 
        if args.partitionR == None:
            ZRDD = sc.textFile(args.initRDD+"_ZRDD").map(eval).partitionBy(args.Nrowcol).persist(StorageLevel.MEMORY_ONLY)
        else:
   ...
   #initialize rho parameters (if adaptrho not passed these value stay fixed).
	...
	if DEBUG:
	   logger.debug("Iteration %d all var pairs is:\n%s" %(iteration,pformat(list(allvars.sortByKey().collect()),width=30)) )
        ...
        if args.partitionR == None:
            ZRDD = allvars.reduceByKey(lambda (value1,count1),(value2,count2) : (value1+value2,count1+count2)  ).mapValues(lambda (value,count): 1.0*value/count).partitionBy(args.Nrowcol).persist(StorageLevel.MEMORY_ONLY)
        else:
	    ZRDD = allvars.reduceByKey(lambda (value1,count1),(value2,count2) : (value1+value2,count1+count2)  ).mapValues(lambda (value,count): 1.0*value/count).partitionBy(args.N, partitionFunc=ZRDDpartFunc).persist(StorageLevel.MEMORY_ONLY)
	...
	if (not (args.silent or args.lean)) or (args.silent and forceComp):
           ...
           if args.partitionR == None:
               ZRDDjoinedOldZRDD = ZRDD.join(oldZ,numPartitions=args.Nrowcol).cache()
           else: 
