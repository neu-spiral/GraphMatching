...
from Attributes import get_all_neighborhoods
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = 'Neighborhoods Algorithm',formatter_class=argparse.ArgumentDefaultsHelpFormatter)    
    ...
    parser.add_argument('k',type=int,default=2,help='Distance of largest neighborhood computed. E.g. k=2 computes the size of the 2-degree neibhborhood.')
    parser.add_argument('--output', default="output",help='File in which neighborhood sizes are stored')
    ...
    sc = SparkContext(appName='Cycles and Neighborhoods', conf=configuration)
    ...
    all_neighborhoods = get_all_neighborhoods(graph,args.N,args.k)
    all_neighborhoods.saveAsTextFile(args.output)
